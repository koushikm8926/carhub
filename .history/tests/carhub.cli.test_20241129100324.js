...
describe('CLI App', () => {
  ...
  it('TC14', async () => {
    ...
    // Step 2: Run the `add` command
    const { stdout } = await execa('node', [CLI_PATH, 'search', 'Murcielago', testFilePath]);
    
    
    const tableData = [
      ['id', 'model', 'brand', 'colour', 'price', 'units', 'sold'],
      ...[{ "id": 7, "model": "Murcielago", "brand": "Lamborghini", "colour": "black", "price": 70000, "units": 2, "sold": 1 }].map(car => [car.id, car.model, car.brand, car.colour, car.price, car.units, car.sold])
    ];
    const expectedTable = table(tableData);
    expect(stdout.trim()).toBe(expectedTable.trim());

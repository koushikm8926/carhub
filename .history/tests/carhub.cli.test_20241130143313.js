...
it('TC21', async () => {
  ...
  const toAddData = [
    { "id": 1, "model": "i30", "brand": "Hyundai", "colour": "blue", "price": 30000, "units": 3, "sold": 1 }, // Same car as in testFile.json
    { "id": 3, "model": "ModelA", "brand": "Toyota", "colour": "green", "price": 25000, "units": 4, "sold": 1 }
  ];
  fs.writeFileSync(toAddFilePath, JSON.stringify(toAddData, null, 2));
...
it('TC27', async () => {
  ...
  const expectedTable = [
    ['ranking', 'id', 'model', 'brand', 'colour', 'price', 'Price', 'US'],
...
it('TC29', async () => {
  ...
  const expectedOutput = table([`ranking`, `id`, `model`, `brand`, `colour`, `price`, `units`, `sold`]);

...
describe('CLI App', () => {
  ...
  it('TC6', async () => {
    ...
    await execa('node', ['carhub', 'add', 'Aventador', 'Lamborghini', 'yellow', '30000', '3', testFilePath]);
    
    // Step 3: Read and verify the JSON file
    const newCar = { "id": 5, "model": "i30", "brand": "hyundai", "colour": "blue", "price": 30000, "units": 3, "sold": 0 };
  ...
  it('TC7', async () => {
    ...
    // Step 2: Run the `add` command
    await execa('node', ['carhub', 'add', 'i30', 'Aventador', 'blue', '30000', '3', testFilePath]);
    
    // Step 3: Read and verify the JSON file
    const newCar = { "id": 1, "model": "i30", "brand": "hyundai", "colour": "blue", "price": 30000, "units": 3, "sold": 0 };
    const updatedData = JSON.parse(fs.readFileSync(testFilePath, 'utf8'));
    expect(updatedData).toEqual([...initialData, newCar]);
